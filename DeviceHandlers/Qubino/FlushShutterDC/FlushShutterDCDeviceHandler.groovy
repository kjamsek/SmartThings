 ...
 *
 *
 * |---------------------------- DEVICE HANDLER FOR QUBINO FLUSH SHUTTER DC Z-WAVE DEVICE -------------------------------------------------------|  
 *	The handler supports all unsecure functions of the Qubino Flush Shutter DC device. Configuration parameters and
 ...
 *  - Implement Multichannel associations and temperature sensor support
 *
 *	CHANGELOG:
...
metadata {
	definition (name: "Qubino Flush Shutter DC", namespace: "Goap", author: "Kristjan Jam&scaron;ek") {
        ...
        fingerprint mfr:"0159", prod:"0003", model:"0053"  //Manufacturer Information value for Qubino Flush Shutter DC
	}
...
*/
				...
				input name: "param10", type: "enum", required: false,
					...
					title: "10. Activate / deactivate functions ALL ON / ALL OFF.\n " +
						   "Available settings:\n" +
							...
							"Default value: 255.\n" +
							"Flush Shutter DC module responds to commands ALL ON / ALL OFF that may be sent by the main controller or by other controller belonging to the system."
				...
				input name: "param85", type: "number", range: "3..50", required: false,
					title: "85. Power consumption max delay time.\n" +
						   ...
						   "Default value: 8 = 0,8 seconds."
						   
				input name: "param86", type: "number", range: "3..50", required: false,
					title: "86. Power consumption at limit switch delay time.\n" +
						   "This parameter defines the max time at limit switch, when power consumption is below power threshold (par. 76), the active output will switch off, means that limit switch is reached.\n" +
						   "Available settings:\n" +
						   "3 - 50 = 0,3 seconds - 5 seconds (100ms resolution).\n" +
						   "Default value: 8 = 0,8 seconds."
...
*/
				...
				input name: "assocGroup7", type: "text", required: false,
					title: "Association group 7: \n" +
						   "Multilevel set (triggered at changes of value of the Flush Shutter DC position) up to 16 nodes.\n" +
						   "NOTE: Insert the node Id value of the devices you wish to associate this group with. Multiple nodeIds can also be set at once by separating individual values by a comma (2,3,...)."
...
*/
def configure() {
	log.debug "Qubino Flush Shutter DC: configure()"
...
*/
def stop() {
	log.debug "Qubino Flush Shutter DC: stop()"
	def cmds = []
...
*/
def stopSlats(){
	log.debug "Qubino Flush Shutter DC: stopSlats()"
	def cmds = []
...
*/
def open() {
	log.debug "Qubino Flush Shutter DC: open()"
...
*/
def close() {
	log.debug "Qubino Flush Shutter DC: close()"
...
*/
def openSlats(){
	log.debug "Qubino Flush Shutter DC: openSlats()"
...
*/
def closeSlats(){
	log.debug "Qubino Flush Shutter DC: closeSlats()"
...
def setSlatLevel(level) {
	log.debug "Qubino Flush Shutter DC: setSlatLevel()"
	log.debug level
...
*/
def calibrate() {
	log.debug "Qubino Flush Shutter DC: calibrate()"
...
def refreshPowerConsumption() {
	log.debug "Qubino Flush Shutter DC: refreshPowerConsumption()"
	delayBetween([
...
*/
def resetPower() {
	log.debug "Qubino Flush Shutter DC: resetPower()"
...
def setAssociation() {
	log.debug "Qubino Flush Shutter DC: setAssociation()"
...
def setConfiguration() {
	log.debug "Qubino Flush Shutter DC: setConfiguration()"
	def configSequence = []
	...
	if(settings.param85 != null){
		configSequence << zwave.configurationV1.configurationSet(parameterNumber: 86, size: 1, scaledConfigurationValue: settings.param86.toInteger()).format()
	}
...
def parse(String description) {
	log.debug "Qubino Flush Shutter DC: Parsing '${description}'"
	def result = null
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd){
	log.debug "Qubino Flush Shutter DC: SensorMultilevelReport handler fired"
	def resultEvents = []
...
def zwaveEvent(physicalgraph.zwave.commands.switchmultilevelv3.SwitchMultilevelReport cmd){
	log.debug "Qubino Flush Shutter DC: firing switch multilevel event"
	def result = []
...
def zwaveEvent(physicalgraph.zwave.commands.switchmultilevelv3.SwitchMultilevelReport cmd, physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap command){
	log.debug "Qubino Flush Shutter DC: firing MC switch multilevel event"
	def result = []
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd, physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap command){
	log.debug "Qubino Flush Shutter DC: firing MC Encap meter report event"
	def result = []
	switch(cmd.scale){
		case 0:
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd) {
	log.debug "Qubino Flush Shutter DC: firing meter report event"
	def result = []
	switch(cmd.scale){
		case 0:
			result << createEvent(name:"kwhConsumption", value: cmd.scaledMeterValue, unit:"kWh", descriptionText:"${device.displayName} consumed ${cmd.scaledMeterValue} kWh", isStateChange: true)
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv2.ConfigurationReport cmd){
	log.debug "Qubino Flush Shutter DC: firing configuration report event"
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd){
	log.debug "Qubino Flush Shutter DC: firing basic report event"
	def result = []
	result << createEvent(name:"windowShade", value: cmd.value ? "open" : "closed", isStateChange: true)
	if(cmd.value > 99){
		result << createEvent(name:"level", value: cmd.value, unit:"%", descriptionText:"${device.displayName} is uncalibrated! Please press calibrate!", isStateChange: true)
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCapabilityReport cmd){
	log.debug "Qubino Flush Shutter DC: firing MultiChannelCapabilityReport"
	state.isMcDevice = true
	def cmds = []
	cmds << response(zwave.associationV1.associationRemove(groupingIdentifier:1).format())
	//cmds << "delay 500"
