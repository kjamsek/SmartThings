...
metadata {
	...
	tiles(scale: 2) {
		...
		standardTile("temperatureCh1", "device.temperatureCh1", width: 3, height: 3) {
			state("temperatureCh1", label:'Temp. Ch1: ${currentValue} °F', unit:'°F', icon: 'st.Weather.weather2')
		}
		standardTile("windDirection", "device.windDirection", width: 3, height: 3) {
			state("windDirection", label:'W. Dir.: ${currentValue}', unit: "", icon: 'st.Outdoor.outdoor20')
		}

		standardTile("windVelocity", "device.windVelocity", width: 3, height: 3) {
			state("windVelocity", label:'W. Vel.: ${currentValue} mph', unit:"mph", icon: 'st.Weather.weather1')
		}
		
		standardTile("windGust", "device.windGust", width: 3, height: 3) {
			state("windGust", label:'W. Gust: ${currentValue} mph', unit:"mph", icon: 'st.Weather.weather1')
		}
		standardTile("windTemperature", "device.windTemperature", width: 3, height: 3) {
			state("windTemperature", label:'Out Temp. ${currentValue} °F', unit:'°F', icon: 'st.Weather.weather2')
		}
		standardTile("windChillTemperature", "device.windChillTemperature", width: 3, height: 3) {
			state("windChillTemperature", label:'W. Chill: ${currentValue} °F', unit:'°F', icon: 'st.Weather.weather2')
		}
		...
		standardTile("humidityCh1", "device.humidityCh1", width: 3, height: 3) {
			state("humidityCh1", label:'Hum. Ch1: ${currentValue} %', unit:"%", icon: 'st.Weather.weather12')
		}
		standardTile("temperatureCh2", "device.temperatureCh2", width: 3, height: 3) {
			state("temperatureCh2", label:'Temp. Ch2: ${currentValue} °F', unit:'°F', icon: 'st.Weather.weather2')
		}
		standardTile("humidityCh2", "device.humidityCh2", width: 3, height: 3) {
			state("humidityCh2", label:'Hum. Ch2: ${currentValue} %', unit:"%", icon: 'st.Weather.weather12')
		}
...
def zwaveEvent(physicalgraph.zwave.commands.multichannelv3.MultiChannelCmdEncap cmd) {
	...
	switch (cmd.sourceEndPoint) {
		default:
				break;
		case 1:
				resultEvents << createEvent(name:"temperatureCh1", value: convertDegrees(tempScale,encapsulatedCommand), unit:" °"+location.temperatureScale, descriptionText: "Temperature Ch1: "+convertDegrees(tempScale,encapsulatedCommand)+" °F")
				resultEvents << createEvent(name:"temperature", value: convertDegrees(tempScale,encapsulatedCommand), unit:" °"+location.temperatureScale, descriptionText: "Temperature Ch1: "+convertDegrees(tempScale,encapsulatedCommand)+" °F", displayed: false)
				break;
		case 2:
